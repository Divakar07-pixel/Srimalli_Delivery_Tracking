-- Migration 0011: per-order billing address and repeat-customer defaults
-- Billing information belongs to the order; customer profile data remains reusable.
alter table public.orders
  add column if not exists billing_address text;

-- Backfill existing orders from the linked customer address.
update public.orders o
set billing_address = c.address
from public.customers c
where c.id = o.customer_id
  and o.billing_address is null;
...
begin
  ...
  select json_build_object(
    ...
    'billing_address', coalesce(v_order.billing_address, c.address),
